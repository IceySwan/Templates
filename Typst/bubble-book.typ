...
#let bubble(
  ...
  logo: none,
  main-color: "E94845",//"#4DA6FF", //
  alpha: 60%,
  color-words: (),
  body,
  show-title-page: true, // 新增参数控制是否显示标题页
) = {
  set document(author: author, title: title)

  // Save heading and body font families in variables.
  let body-font = "LXGW WenKai"
  let title-font = "LXGW WenKai"
  ...
  if show-title-page {
    ...
    // Display title, author, etc. at the top of the first page
    align(center, text(font: title-font, 2em, weight: 700, title))
    if subtitle != none {
      v(1em, weak: true)
      align(center, text(font: title-font, 1.5em, weight: 700, subtitle))
    }
    v(1em, weak: true)
    align(center, text(1em, date))
    v(1em, weak: true)
    align(center)[
      #if author != "" {strong(author); linebreak();}
      #if affiliation != none {affiliation; linebreak();}
